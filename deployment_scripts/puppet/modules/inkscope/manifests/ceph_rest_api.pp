class inkscope::ceph_rest_api (
  ...
  exec {'radogw-admin':
      command => "/usr/bin/radosgw-admin user create --uid=inkscope --display-name=\"Inkscope Admin RGW\" --email=root@example.com --secret=inkscopeadmsecret --access-key=inkscopeadmkey --system",
